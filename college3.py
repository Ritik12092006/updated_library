...
# --- 6. AI ENGINE (FULL POWER) ---
@st.cache_resource
def setup_ai_engine(data_descriptions):
    tfidf = TfidfVectorizer(stop_words='english')
    # Full dataset read (No Limit)
    tfidf_matrix = tfidf.fit_transform(data_descriptions.fillna('')) 
...
def chat_with_library(query):
    try:
        ...
        if similarity[top_indices[0]] < 0.1: return "Try asking about Engineering, Medical, or Entrance Exams."
        ...
        return response
    except: return "I am analyzing the library..."
